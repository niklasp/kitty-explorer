...
const NFT_BY_ID_ENDPOINT = 'https://rmrklistener.com/get_nft_by_id/';
...
export async function getKitties( orderby = 'id', order='desc' ) {
  const response = await fetch("https://rmrklistener.com/get_nfts_by_collection/800f8a914281765a7d-KITTY",  { agent } );
  ...
  const realKittyData = data
  .filter( kitObj => kitObj.burned === '')
  .map( ( kitObj ) => {
    const {
      id,
      metadata,
