...
export async function getSortedKitties() {
  ...
  // const fetchData = async () => {
    ...
    console.log( 'fetched the data again' );
    const data = await response.json();
  //   return data;
  // }

  // const realKittyData = await fetchData();
  ...
  const realKittyData = data.map( ( kitObj ) => {
    ...
    // console.log( 'aaaa', kitObj.id, kitObj.metadata );
    try {
      metadata.slice(1, -1);
      parsedMeta = JSON.parse( metadata );
      // console.log( 'success parsing metadata for ', kitObj.id, Object.keys( parsedMeta ) );
    } catch( e ) {
      console.log( 'error parsing metadata for ', kitObj.id );
    }

    // console.log( 'metadata is', parsedMeta );

    // console.log( kitObj.did_fetch_metadata );
