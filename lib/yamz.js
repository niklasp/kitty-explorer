...
export function customLightboxGenerator(
  ...
  $descText.innerHTML = md.render($original.dataset.description);
  ...
  if ( $original.dataset.id && $original.dataset.id !== '' ) {
    ...
    if ( $original.dataset.forsale && $original.dataset.forsale !== '0' ) {
      $singularButton.appendChild(document.createTextNode(`Buy for ${ $original.dataset.price / 0.9 / 1000000000000 } KSM` ));
    } else {
